...
# --- 核心改动：修改 DirGCNConv，使用 alpha 加权融合 ---
class DirGCNConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        # 使用 alpha 加权融合
        return self.alpha * x_forward + (1 - self.alpha) * x_backward


# --- 修改 get_conv，传递 alpha ---
...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        # GNN 初始化，传递 alpha 和 learn_alpha
        self.gnn = GNN(
            num_features=self.hidden_size, hidden_dim=self.hidden_size,
            num_layers=opt.step, dropout=0.0, conv_type="dir-gcn",
            jumping_knowledge=opt.jk, alpha=0.5, learn_alpha=True  # 使用 learn_alpha=True 以学习 alpha
        )
