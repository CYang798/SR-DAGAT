...
class Data():
    ...
    def get_slice(self, i):
        ...
        # 将批次中的唯一物品ID映射到一个从0开始的连续索引
        unique_items_map, inverse_indices = np.unique(all_unique_items_in_batch, return_inverse=True)
        unique_items_map_list = unique_items_map.tolist()

        # 将原始的 inputs 转换为指向 unique_items_map 索引的 alias_inputs
        item_to_idx = {item: i for i, item in enumerate(unique_items_map_list)}

        new_alias_inputs = []
        for u_input in inputs:
            new_alias_inputs.append([item_to_idx[item] if item != 0 else 0 for item in u_input])
        ...
        for u_input in inputs:
            for k in range(len(u_input) - 1):
                ...
                if src_item_id in item_to_idx and dst_item_id in item_to_idx:
                    src_global_idx = item_to_idx[src_item_id]
                    dst_global_idx = item_to_idx[dst_item_id]
        ...
        # GNN的输入节点应该是唯一的物品ID
        # 注意：这里我们传递原始的物品ID（unique_items_map），而不是它们的索引
        # embedding层会处理ID到嵌入的映射
        # GNN处理完后，我们使用new_alias_inputs来索引回序列结构
