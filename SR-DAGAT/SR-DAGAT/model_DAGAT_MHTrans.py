...
# --- 改进：DAGCNConv，使用多头注意力融合，并添加残差和LayerNorm ---
class DAGCNConv(torch.nn.Module):
    def __init__(self, input_dim, output_dim, heads=4):  # 新增：heads参数，默认4头注意力
        ...
        # --- 改进：使用多头自注意力代替简单MLP ---
        self.attention = MultiheadAttention(embed_dim=output_dim * 2, num_heads=heads, dropout=0.1)  # 输入拼接，dropout防过拟合
    ...
    def forward(self, x, edge_index):
        ...
        # --- 改进：使用多头自注意力融合 ---
        combined_repr = torch.cat([x_forward, x_backward], dim=-1).unsqueeze(0)  # [1, N, 2*D] for MHA
        attn_output, _ = self.attention(combined_repr, combined_repr, combined_repr)
        output = attn_output.squeeze(0)[:, :self.output_dim]  # 只取前半部分作为输出，或平均

        # --- 新增：残差连接和LayerNorm ---
        output = self.norm(output + x_forward)  # 残差以x_forward为主，或自定义
        output = self.dropout(output)

        return output

# --- 修改 get_conv，传递 heads ---
def get_conv(conv_type, input_dim, output_dim, heads=4):
    if conv_type == "dir-gcn":
        return DAGCNConv(input_dim, output_dim, heads)
    else:
        raise ValueError(f"Convolution type {conv_type} not supported")

# --- 改进：GNN，添加LayerNorm和残差在层间 ---
class GNN(torch.nn.Module):
    def __init__(
        ...
        if num_layers == 1:
            self.convs = ModuleList([get_conv(conv_type, num_features, output_dim, heads)])
        else:
            self.convs = ModuleList([get_conv(conv_type, num_features, hidden_dim, heads)])
            for _ in range(num_layers - 2):
                self.convs.append(get_conv(conv_type, hidden_dim, hidden_dim, heads))
            self.convs.append(get_conv(conv_type, hidden_dim, output_dim, heads))
        ...
        # --- 新增：层间LayerNorm ---
        self.layer_norms = ModuleList([LayerNorm(hidden_dim) for _ in range(num_layers)])
    ...
    def forward(self, x, edge_index):
        ...
        for i, conv in enumerate(self.convs):
            ...
            x = self.layer_norms[i](x + residual)  # 层间残差 + Norm
            xs += [x]
        if self.jumping_knowledge and self.jumping_knowledge != '':
            x = self.jump(xs)
            x = self.lin(x)
...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        # --- 新增：Transformer Encoder层，用于序列处理 ---
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.hidden_size, nhead=opt.heads, dropout=0.1)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1)  # 单层Encoder

        # GNN 初始化传递 heads
        self.gnn = GNN(
            num_features=self.hidden_size, hidden_dim=self.hidden_size,
            num_layers=opt.step, dropout=0.0, conv_type="dir-gcn",
            jumping_knowledge=opt.jk, heads=opt.heads
        )
...
# --- 改进：在外部 forward 函数中，添加Transformer Encoder处理序列 ---
def forward(model, i, data):
    ...
    # 4. --- 新增：使用Transformer Encoder处理序列（src_key_padding_mask基于mask） ---
    src_key_padding_mask = (mask == 0)  # padding位置为True
    seq_hidden = seq_hidden.permute(1, 0, 2)  # [seq_len, batch, dim] for Transformer
    seq_hidden = model.transformer_encoder(seq_hidden, src_key_padding_mask=src_key_padding_mask)
    seq_hidden = seq_hidden.permute(1, 0, 2)  # 恢复 [batch, seq_len, dim]
