...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        # --- 新增：对比学习相关参数 ---
        self.tau = 0.1  # 温度参数
        self.cl_weight = opt.cl_weight  # 对比损失权重，从opt传入
    ...
    # --- 新增：计算对比损失（InfoNCE） ---
    def compute_contrastive_loss(self, hidden, mask):
        # 使用最后有效表示作为锚点
        anchors = hidden[torch.arange(mask.shape[0]).long(), torch.sum(mask, 1) - 1]
        anchors = F.normalize(anchors, p=2, dim=1)

        # 正样本：批次内相似序列（这里简单用自身作为正样本，实际可增强）
        positives = anchors  # 为简单，可添加数据增强如随机掩码

        # 负样本：批次内其他样本
        sim_matrix = torch.matmul(anchors, anchors.T) / self.tau
        mask_pos = torch.eye(sim_matrix.size(0), device=sim_matrix.device)  # 对角线为正
        sim_matrix = sim_matrix - 1e9 * mask_pos  # 屏蔽自身作为负

        # InfoNCE损失
        logits = F.log_softmax(sim_matrix, dim=1)
        cl_loss = -torch.mean(torch.diag(logits))  # 简化，实际可扩展负采样
        return cl_loss
...
def forward(model, i, data):
    ...
    # --- 新增：计算对比损失 ---
    cl_loss = model.compute_contrastive_loss(seq_hidden, mask)

    return targets, scores, cl_loss

# --- 修改 train_test：加入对比损失到总损失 ---
def train_test(model, train_data, test_data):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        targets, scores, cl_loss = forward(model, i, train_data)
        targets = trans_to_cuda(torch.Tensor(targets).long())
        ce_loss = model.loss_function(scores, targets - 1)
        loss = ce_loss + model.cl_weight * cl_loss  # 加权结合
        loss.backward()
        model.optimizer.step()
        total_loss += loss.item()
        if j % int(len(slices) / 5 + 1) == 0:
            print('[%d/%d] Loss: %.4f (CE: %.4f, CL: %.4f)' % (j, len(slices), loss.item(), ce_loss.item(), cl_loss.item()))
    ...
    for i in slices:
        targets, scores, _ = forward(model, i, test_data)  # 测试时忽略cl_loss
