...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        # --- 新增：GRU层来建模序列 ---
        # batch_first=True 表示输入的维度是 (batch_size, seq_len, hidden_size)
    ...
    def compute_scores(self, hidden, mask):
        # 当前兴趣（局部偏好）：获取最后一个物品的GRU隐藏状态
        last_item_idx = torch.sum(mask, 1) - 1
        ht = hidden[torch.arange(mask.shape[0]).long(), last_item_idx]  # (batch_size, hidden_size)

        # 全局兴趣（长期偏好）：对GRU所有步的输出进行注意力加权
        ...
        # 加权求和
        s_g = torch.sum(alpha * hidden * mask.view(mask.shape[0], -1, 1).float(), 1)  # (batch_size, hidden_size)

        if not self.nonhybrid:
            # --- 经典融合方式：拼接后线性变换 ---
            final_preference = self.linear_transform(torch.cat([s_g, ht], 1))
        else:
    ...
    def forward(self, alias_inputs, edge_index, items, mask):
        ...
        seq_hidden_with_pos = seq_hidden_no_pos + pos_embedding * mask.unsqueeze(2).float()

        # 4. --- 新增：将序列通过GRU层，学习时序依赖 ---
        # GRU的输出包含了所有时间步的隐藏状态
        gru_output, _ = self.gru(seq_hidden_with_pos)  # gru_output shape: (batch_size, seq_len, hidden_size)
