...
class DAGCNConv(torch.nn.Module):
    def __init__(self, input_dim, output_dim, alpha=None):  # alpha 不再使用
        ...
        # 如果输入和输出维度不同，需要一个线性层来匹配维度以便残差连接
        if input_dim != output_dim:
            self.residual_linear = Linear(input_dim, output_dim, bias=False)
        else:
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        xs = []
        # Store initial input for the first residual connection if needed
        x_in = x
