...
from torch.nn import ModuleList, Linear, LayerNorm, MultiheadAttention


# --- directed_norm, get_norm_adj, DAGCNConv, get_conv, GNN 保持不变 ---
...
class DAGCNConv(torch.nn.Module):
    def __init__(self, input_dim, output_dim, alpha=None):
...
def get_conv(conv_type, input_dim, output_dim, alpha=None):
    if conv_type == "dir-gcn":
        return DAGCNConv(input_dim, output_dim)
    else:
...
class TransformerLayer(nn.Module):
    def __init__(self, hidden_size, n_heads=4, dropout=0.1):
        ...
        self.self_attn = MultiheadAttention(hidden_size, n_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, x, mask):
        # PyTorch的MultiheadAttention需要一个布尔类型的key_padding_mask
        # mask中1代表有效数据, 0代表padding, 而mask需要True代表被mask掉(padding)
        attn_mask = (mask == 0)
...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        # --- 新增：Transformer层 ---
        self.transformer_layer = TransformerLayer(self.hidden_size, n_heads=opt.n_heads,
        ...
        # 新增门控融合机制
        self.gate_linear = nn.Linear(self.hidden_size * 2, self.hidden_size)
    ...
    def compute_scores(self, hidden, mask):
        # 局部偏好：获取最后一个物品的表示
        last_item_idx = torch.sum(mask, 1) - 1
        ht = hidden[torch.arange(mask.shape[0]).long(), last_item_idx]

        # 全局偏好：对Transformer输出的序列表示进行注意力加权
        ...
        if not self.nonhybrid:
            # --- 使用门控机制融合全局和局部偏好 ---
            combined = torch.cat([s_g, ht], dim=1)
            gate = torch.sigmoid(self.gate_linear(combined))
            final_preference = gate * s_g + (1 - gate) * ht
        else:
            final_preference = s_g

        # 计算得分
        b = self.embedding.weight[1:]
        scores = torch.matmul(final_preference, b.transpose(1, 0))
        return scores

    def forward(self, alias_inputs, edge_index, items, mask):
        # 1. GNN 在唯一物品图上进行传播，得到上下文感知的物品嵌入
        # 注意：这里需要传入原始物品ID的嵌入
        item_embeddings = self.embedding(items)
        graph_node_hidden = self.gnn(item_embeddings, edge_index)
        ...
        pos_embedding = self.position_embedding(positions).unsqueeze(0)
        # 通过 mask 确保不给 padding 加位置信息
        seq_hidden = seq_hidden_no_pos + pos_embedding * mask.unsqueeze(2).float()

        # 4. --- 新增：将序列通过Transformer层，学习序列内部依赖 ---
        final_seq_hidden = self.transformer_layer(seq_hidden, mask)

        return final_seq_hidden
...
def forward(model, i, data):
    ...
    # 模型内部完成 GNN->Pos->Transformer 的完整流程
    hidden = model(alias_inputs, edge_index, items, mask)
